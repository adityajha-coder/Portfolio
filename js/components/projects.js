export function renderProjects(projects) {
    ...
    projectGrid.className = 'relative w-full sm:max-w-xl md:max-w-3xl lg:max-w-4xl mx-auto md:px-16';
    projectGrid.style.setProperty('--swiper-theme-color', '#f472b6');
    projectGrid.style.setProperty('--swiper-pagination-bottom', '0px');
    ...
    projects.forEach((project, index) => {
        ...
        slidesHTML += `
            <div class="swiper-slide pb-10 md:pb-0"> <!-- Added padding bottom on mobile for dots -->
    ...
    projectGrid.innerHTML = `
        <div class="swiper project-swiper w-full rounded-2xl overflow-hidden shadow-2xl">
            <div class="swiper-wrapper">
                ${slidesHTML}
            </div>
            <!-- Pagination dots (visible mainly on mobile) -->
            <div class="swiper-pagination md:hidden"></div>
        </div>
        <!-- Buttons hidden on mobile, visible on desktop -->
        <div class="swiper-button-next hidden md:flex" style="right: 0;"></div>
        <div class="swiper-button-prev hidden md:flex" style="left: 0;"></div>
    `;
    ...
    if (typeof window.Swiper !== 'undefined') {
        ...
        console.error('Swiper is not loaded or defined.');
    }
}